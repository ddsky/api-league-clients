...
(defn-spec find-similar-games-api-with-http-info any?
  ...
  ([id int?, ] (find-similar-games-api-with-http-info id nil))
  ([id int?, {:keys [limit]} (s/map-of keyword? any?)]
   (check-required-params id)
   (call-api "/list-similar-games" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"id" id "limit" limit }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["apiKey" "headerApiKey"]})))

(defn-spec find-similar-games-api find-similar-games-api-200-response-spec
  "Find Similar Games API
  Find similar games based on a given game using AI. This API allows you to retrieve a list of games that are similar to a specified game, providing recommendations based on various factors such as genre, gameplay mechanics, and user preferences."
  ([id int?, ] (find-similar-games-api id nil))
  ([id int?, optional-params any?]
   (let [res (:data (find-similar-games-api-with-http-info id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode find-similar-games-api-200-response-spec res st/string-transformer)
        res))))


(defn-spec retrieve-game-by-id-with-http-info any?
  "Retrieve Game by Id
...
(defn-spec retrieve-game-by-id retrieve-game-by-id-200-response-spec
  "Retrieve Game by Id
  This API allows you to retrieve detailed information about a specific game by its unique identifier. Game details include the game's name, description, release date, developer, platforms, genres, tags, ratings, screenshots, videos, and more."
  [id int?]
  (let [res (:data (retrieve-game-by-id-with-http-info id))]
    (if (:decode-models *api-context*)
       (st/decode retrieve-game-by-id-200-response-spec res st/string-transformer)
       res)))


(defn-spec search-games-api-with-http-info any?
  "Search Games API
  ...
  ([] (search-games-api-with-http-info nil))
  ([{:keys [query offset limit filters sort sort-order generate-filter-options]} (s/map-of keyword? any?)]
   (call-api "/search-games" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"query" query "offset" offset "limit" limit "filters" filters "sort" sort "sort-order" sort-order "generate-filter-options" generate-filter-options }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["apiKey" "headerApiKey"]})))

(defn-spec search-games-api search-games-api-200-response-spec
  "Search Games API
  Search through a vast database of over half a million video games from more than 50 platforms. This API allows you to find games based on various criteria such as genre, platform, release date, and more. The results include detailed information about each game, including ratings, descriptions, and images."
  ([] (search-games-api nil))
  ([optional-params any?]
   (let [res (:data (search-games-api-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode search-games-api-200-response-spec res st/string-transformer)
        res))))
