...
module APILeague.API.News where
...
extractNewsAPI
  ...
  -> APILeagueRequest ExtractNewsAPI MimeNoContent ExtractNewsAPI200Response MimeJSON
extractNewsAPI (Url url) (Analyze analyze) =
...
data ExtractNewsAPI  
-- | @application/json@
instance Produces ExtractNewsAPI MimeJSON
...
-- 
searchNewsAPI
  :: APILeagueRequest SearchNewsAPI MimeNoContent SearchNewsAPI200Response MimeJSON
searchNewsAPI =
...
-- | /Optional Param/ "text" - The text to match in the news content (at least 3 characters, maximum 100 characters). By default all query terms are expected, you can use an uppercase OR to search for any terms, e.g. tesla OR ford. You can also exclude terms by putting a minus sign (-) in front of the term, e.g. tesla -ford.
instance HasOptionalParam SearchNewsAPI ParamText where
...
instance HasOptionalParam SearchNewsAPI SourceCountries where
...
instance HasOptionalParam SearchNewsAPI Language where
...
instance HasOptionalParam SearchNewsAPI MinSentiment where
...
instance HasOptionalParam SearchNewsAPI MaxSentiment where
...
instance HasOptionalParam SearchNewsAPI EarliestPublishDate where
...
instance HasOptionalParam SearchNewsAPI LatestPublishDate where
...
instance HasOptionalParam SearchNewsAPI NewsSources where
...
instance HasOptionalParam SearchNewsAPI Authors where
...
instance HasOptionalParam SearchNewsAPI Categories where
...
instance HasOptionalParam SearchNewsAPI Entities where
  applyOptionalParam req (Entities xs) =
    req `addQuery` toQuery ("entities", Just xs)

-- | /Optional Param/ "location-filter" - Filter news by radius around a certain location. Format is \"latitude,longitude,radius in kilometers\"
instance HasOptionalParam SearchNewsAPI LocationFilter where
...
instance HasOptionalParam SearchNewsAPI Sort where
...
instance HasOptionalParam SearchNewsAPI SortDirection where
...
topNewsAPI
  ...
  -> APILeagueRequest TopNewsAPI MimeNoContent TopNewsAPI200Response MimeJSON
topNewsAPI (SourceCountry sourceCountry) (Language language) =
  _mkRequest "GET" ["/retrieve-top-news"]
    `_hasAuthType` (P.Proxy :: P.Proxy AuthApiKeyApiKey)
    `_hasAuthType` (P.Proxy :: P.Proxy AuthApiKeyHeaderApiKey)
...
instance HasOptionalParam TopNewsAPI ParamDate where
...
instance HasOptionalParam TopNewsAPI HeadlinesOnly where
  applyOptionalParam req (HeadlinesOnly xs) =
    req `addQuery` toQuery ("headlines-only", Just xs)
-- | @application/json@
instance Produces TopNewsAPI MimeJSON
