...
module OpenapiClient
  class MediaApi
    ...
    def search_icons(query, opts = {})
      data, _status_code, _headers = search_icons_with_http_info(query, opts)
      data
    end
    ...
    # @option opts [Boolean] :only_public_domain If true, only public domain icons will be returned.
    # @option opts [Integer] :number The number of icons to return in range [1,100]
    # @return [Array<(SearchIcons200Response, Integer, Hash)>] SearchIcons200Response data, response status code and response headers
    def search_icons_with_http_info(query, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: MediaApi.search_icons ...'
      end
      # verify the required parameter 'query' is set
      if @api_client.config.client_side_validation && query.nil?
        fail ArgumentError, "Missing the required parameter 'query' when calling MediaApi.search_icons"
      end
      if @api_client.config.client_side_validation && query.to_s.length > 300
        fail ArgumentError, 'invalid value for "query" when calling MediaApi.search_icons, the character length must be smaller than or equal to 300.'
      end

      pattern = Regexp.new(/./)
      if @api_client.config.client_side_validation && query !~ pattern
        fail ArgumentError, "invalid value for 'query' when calling MediaApi.search_icons, must conform to the pattern #{pattern}."
      end

      if @api_client.config.client_side_validation && !opts[:'number'].nil? && opts[:'number'] > 100
        fail ArgumentError, 'invalid value for "opts[:"number"]" when calling MediaApi.search_icons, must be smaller than or equal to 100.'
      end

      if @api_client.config.client_side_validation && !opts[:'number'].nil? && opts[:'number'] < 1
        fail ArgumentError, 'invalid value for "opts[:"number"]" when calling MediaApi.search_icons, must be greater than or equal to 1.'
      end
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'SearchIcons200Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['apiKey', 'headerApiKey']

      new_options = opts.merge(
        :operation => :"MediaApi.search_icons",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: MediaApi#search_icons\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
    ...
    def search_royalty_free_images_with_http_info(query, opts = {})
      ...
      if @api_client.config.client_side_validation && !opts[:'number'].nil? && opts[:'number'] > 100
        fail ArgumentError, 'invalid value for "opts[:"number"]" when calling MediaApi.search_royalty_free_images, must be smaller than or equal to 100.'
      end
