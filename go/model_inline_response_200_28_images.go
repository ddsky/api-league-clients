/*
API League

API League is a Hub for World Class APIs.

API version: 1.0
Contact: mail@apileague.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// InlineResponse20028Images struct for InlineResponse20028Images
type InlineResponse20028Images struct {
	Width *int32 `json:"width,omitempty"`
	License *InlineResponse20028License `json:"license,omitempty"`
	Thumbnail NullableString `json:"thumbnail,omitempty"`
	Id NullableString `json:"id,omitempty"`
	Url NullableString `json:"url,omitempty"`
	Height *int32 `json:"height,omitempty"`
}

// NewInlineResponse20028Images instantiates a new InlineResponse20028Images object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse20028Images() *InlineResponse20028Images {
	this := InlineResponse20028Images{}
	return &this
}

// NewInlineResponse20028ImagesWithDefaults instantiates a new InlineResponse20028Images object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse20028ImagesWithDefaults() *InlineResponse20028Images {
	this := InlineResponse20028Images{}
	return &this
}

// GetWidth returns the Width field value if set, zero value otherwise.
func (o *InlineResponse20028Images) GetWidth() int32 {
	if o == nil || o.Width == nil {
		var ret int32
		return ret
	}
	return *o.Width
}

// GetWidthOk returns a tuple with the Width field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse20028Images) GetWidthOk() (*int32, bool) {
	if o == nil || o.Width == nil {
		return nil, false
	}
	return o.Width, true
}

// HasWidth returns a boolean if a field has been set.
func (o *InlineResponse20028Images) HasWidth() bool {
	if o != nil && o.Width != nil {
		return true
	}

	return false
}

// SetWidth gets a reference to the given int32 and assigns it to the Width field.
func (o *InlineResponse20028Images) SetWidth(v int32) {
	o.Width = &v
}

// GetLicense returns the License field value if set, zero value otherwise.
func (o *InlineResponse20028Images) GetLicense() InlineResponse20028License {
	if o == nil || o.License == nil {
		var ret InlineResponse20028License
		return ret
	}
	return *o.License
}

// GetLicenseOk returns a tuple with the License field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse20028Images) GetLicenseOk() (*InlineResponse20028License, bool) {
	if o == nil || o.License == nil {
		return nil, false
	}
	return o.License, true
}

// HasLicense returns a boolean if a field has been set.
func (o *InlineResponse20028Images) HasLicense() bool {
	if o != nil && o.License != nil {
		return true
	}

	return false
}

// SetLicense gets a reference to the given InlineResponse20028License and assigns it to the License field.
func (o *InlineResponse20028Images) SetLicense(v InlineResponse20028License) {
	o.License = &v
}

// GetThumbnail returns the Thumbnail field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *InlineResponse20028Images) GetThumbnail() string {
	if o == nil || o.Thumbnail.Get() == nil {
		var ret string
		return ret
	}
	return *o.Thumbnail.Get()
}

// GetThumbnailOk returns a tuple with the Thumbnail field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *InlineResponse20028Images) GetThumbnailOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return o.Thumbnail.Get(), o.Thumbnail.IsSet()
}

// HasThumbnail returns a boolean if a field has been set.
func (o *InlineResponse20028Images) HasThumbnail() bool {
	if o != nil && o.Thumbnail.IsSet() {
		return true
	}

	return false
}

// SetThumbnail gets a reference to the given NullableString and assigns it to the Thumbnail field.
func (o *InlineResponse20028Images) SetThumbnail(v string) {
	o.Thumbnail.Set(&v)
}
// SetThumbnailNil sets the value for Thumbnail to be an explicit nil
func (o *InlineResponse20028Images) SetThumbnailNil() {
	o.Thumbnail.Set(nil)
}

// UnsetThumbnail ensures that no value is present for Thumbnail, not even an explicit nil
func (o *InlineResponse20028Images) UnsetThumbnail() {
	o.Thumbnail.Unset()
}

// GetId returns the Id field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *InlineResponse20028Images) GetId() string {
	if o == nil || o.Id.Get() == nil {
		var ret string
		return ret
	}
	return *o.Id.Get()
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *InlineResponse20028Images) GetIdOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return o.Id.Get(), o.Id.IsSet()
}

// HasId returns a boolean if a field has been set.
func (o *InlineResponse20028Images) HasId() bool {
	if o != nil && o.Id.IsSet() {
		return true
	}

	return false
}

// SetId gets a reference to the given NullableString and assigns it to the Id field.
func (o *InlineResponse20028Images) SetId(v string) {
	o.Id.Set(&v)
}
// SetIdNil sets the value for Id to be an explicit nil
func (o *InlineResponse20028Images) SetIdNil() {
	o.Id.Set(nil)
}

// UnsetId ensures that no value is present for Id, not even an explicit nil
func (o *InlineResponse20028Images) UnsetId() {
	o.Id.Unset()
}

// GetUrl returns the Url field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *InlineResponse20028Images) GetUrl() string {
	if o == nil || o.Url.Get() == nil {
		var ret string
		return ret
	}
	return *o.Url.Get()
}

// GetUrlOk returns a tuple with the Url field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *InlineResponse20028Images) GetUrlOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return o.Url.Get(), o.Url.IsSet()
}

// HasUrl returns a boolean if a field has been set.
func (o *InlineResponse20028Images) HasUrl() bool {
	if o != nil && o.Url.IsSet() {
		return true
	}

	return false
}

// SetUrl gets a reference to the given NullableString and assigns it to the Url field.
func (o *InlineResponse20028Images) SetUrl(v string) {
	o.Url.Set(&v)
}
// SetUrlNil sets the value for Url to be an explicit nil
func (o *InlineResponse20028Images) SetUrlNil() {
	o.Url.Set(nil)
}

// UnsetUrl ensures that no value is present for Url, not even an explicit nil
func (o *InlineResponse20028Images) UnsetUrl() {
	o.Url.Unset()
}

// GetHeight returns the Height field value if set, zero value otherwise.
func (o *InlineResponse20028Images) GetHeight() int32 {
	if o == nil || o.Height == nil {
		var ret int32
		return ret
	}
	return *o.Height
}

// GetHeightOk returns a tuple with the Height field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse20028Images) GetHeightOk() (*int32, bool) {
	if o == nil || o.Height == nil {
		return nil, false
	}
	return o.Height, true
}

// HasHeight returns a boolean if a field has been set.
func (o *InlineResponse20028Images) HasHeight() bool {
	if o != nil && o.Height != nil {
		return true
	}

	return false
}

// SetHeight gets a reference to the given int32 and assigns it to the Height field.
func (o *InlineResponse20028Images) SetHeight(v int32) {
	o.Height = &v
}

func (o InlineResponse20028Images) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Width != nil {
		toSerialize["width"] = o.Width
	}
	if o.License != nil {
		toSerialize["license"] = o.License
	}
	if o.Thumbnail.IsSet() {
		toSerialize["thumbnail"] = o.Thumbnail.Get()
	}
	if o.Id.IsSet() {
		toSerialize["id"] = o.Id.Get()
	}
	if o.Url.IsSet() {
		toSerialize["url"] = o.Url.Get()
	}
	if o.Height != nil {
		toSerialize["height"] = o.Height
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse20028Images struct {
	value *InlineResponse20028Images
	isSet bool
}

func (v NullableInlineResponse20028Images) Get() *InlineResponse20028Images {
	return v.value
}

func (v *NullableInlineResponse20028Images) Set(val *InlineResponse20028Images) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse20028Images) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse20028Images) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse20028Images(val *InlineResponse20028Images) *NullableInlineResponse20028Images {
	return &NullableInlineResponse20028Images{value: val, isSet: true}
}

func (v NullableInlineResponse20028Images) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse20028Images) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


