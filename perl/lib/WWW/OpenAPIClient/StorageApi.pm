...
# @param string $key The key for which the value is stored (max length 255 characters). (required)
{
    ...
    __PACKAGE__->method_documentation->{ 'read_key_value_from_store_api' } = {
        summary => 'Read Key Value from Store API',
        params => $params,
        returns => 'ReadKeyValueFromStoreAPI200Response',
        };
}
# @return ReadKeyValueFromStoreAPI200Response
#
sub read_key_value_from_store_api {
    my ($self, %args) = @_;

    # verify the required parameter 'key' is set
    unless (exists $args{'key'}) {
      croak("Missing the required parameter 'key' when calling read_key_value_from_store_api");
    }
    ...
    my $_response_object = $self->{api_client}->deserialize('ReadKeyValueFromStoreAPI200Response', $response);
...
# @param string $value The value that is supposed to be stored (max length 10,000 characters). (required)
{
    ...
    __PACKAGE__->method_documentation->{ 'store_key_value_getapi' } = {
        summary => 'Store Key Value (GET) API',
        params => $params,
        returns => 'StoreKeyValueGETAPI200Response',
        };
}
# @return StoreKeyValueGETAPI200Response
#
sub store_key_value_getapi {
    my ($self, %args) = @_;

    # verify the required parameter 'key' is set
    unless (exists $args{'key'}) {
      croak("Missing the required parameter 'key' when calling store_key_value_getapi");
    }

    # verify the required parameter 'value' is set
    unless (exists $args{'value'}) {
      croak("Missing the required parameter 'value' when calling store_key_value_getapi");
    }
    ...
    my $_response_object = $self->{api_client}->deserialize('StoreKeyValueGETAPI200Response', $response);
    return $_response_object;
}
