...
void OAIArtApi::imageToAsciiArtByURLCallback(OAIHttpRequestWorker *worker) {
    ...
    if (worker->error_type == QNetworkReply::NoError) {
        emit imageToAsciiArtByURLSignal(output);
        emit imageToAsciiArtByURLSignalFull(worker, output);
    } else {
        emit imageToAsciiArtByURLSignalE(output, error_type, error_str);
        emit imageToAsciiArtByURLSignalEFull(worker, error_type, error_str);
    }
}
...
void OAIArtApi::randomPoem(const ::OpenAPI::OptionalParam<qint32> &min_lines, const ::OpenAPI::OptionalParam<qint32> &max_lines) {
    ...
    if (min_lines.hasValue())
    {
        ...
        fullPath.append(QUrl::toPercentEncoding("min-lines")).append(querySuffix).append(QUrl::toPercentEncoding(::OpenAPI::toStringValue(min_lines.value())));
    }
    if (max_lines.hasValue())
    {
        ...
        fullPath.append(QUrl::toPercentEncoding("max-lines")).append(querySuffix).append(QUrl::toPercentEncoding(::OpenAPI::toStringValue(max_lines.value())));
    }
    OAIHttpRequestWorker *worker = new OAIHttpRequestWorker(this, _manager);
    worker->setTimeOut(_timeOut);
    worker->setWorkingDirectory(_workingDirectory);
...
void OAIArtApi::randomPoemCallback(OAIHttpRequestWorker *worker) {
    ...
    if (worker->error_type == QNetworkReply::NoError) {
        emit randomPoemSignal(output);
        emit randomPoemSignalFull(worker, output);
    } else {
        emit randomPoemSignalE(output, error_type, error_str);
        emit randomPoemSignalEFull(worker, error_type, error_str);
    }
}
