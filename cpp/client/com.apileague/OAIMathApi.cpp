...
void OAIMathApi::convertUnitsCallback(OAIHttpRequestWorker *worker) {
    ...
    if (worker->error_type == QNetworkReply::NoError) {
        emit convertUnitsSignal(output);
        emit convertUnitsSignalFull(worker, output);
    } else {
        emit convertUnitsSignalE(output, error_type, error_str);
        emit convertUnitsSignalEFull(worker, error_type, error_str);
    }
}
