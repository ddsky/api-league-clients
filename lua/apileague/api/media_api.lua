...
local apileague_search_icons_200_response = require "apileague.model.search_icons_200_response"
local apileague_search_royalty_free_images_200_response = require "apileague.model.search_royalty_free_images_200_response"
...
function media_api:detect_main_image_color(url)
	...
	if http_status:sub(1,1) == "2" then
		...
		for _, ob in ipairs(result) do
			apileague_detect_main_image_color_200_response_inner.cast(ob)
		end
...
function media_api:search_icons(query, only_public_domain, Number_)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
	...
	if http_status:sub(1,1) == "2" then
		...
		return apileague_search_icons_200_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
...
function media_api:search_royalty_free_images(query, Number_)
	...
	if http_status:sub(1,1) == "2" then
		...
		return apileague_search_royalty_free_images_200_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
