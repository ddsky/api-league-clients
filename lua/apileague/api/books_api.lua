...
-- model import
local apileague_find_similar_books_api_200_response = require "apileague.model.find_similar_books_api_200_response"
local apileague_search_books_api_200_response = require "apileague.model.search_books_api_200_response"
...
local function new_books_api(authority, basePath, schemes)
	local schemes_map = {}
...
function books_api:find_similar_books_api(id, Number_)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/list-similar-books?id=%s&number=%s",
			self.basePath, http_util.encodeURIComponent(id), http_util.encodeURIComponent(Number_));
	})
	...
	if http_status:sub(1,1) == "2" then
		...
		return apileague_find_similar_books_api_200_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
...
function books_api:search_books_api(query, earliest_publish_year, latest_publish_year, min_rating, max_rating, genres, authors, isbn, oclc, sort, sort_direction, group_results, offset, Number_)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
	...
	if http_status:sub(1,1) == "2" then
		...
		return apileague_search_books_api_200_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
