...
-- model import
local apileague_compute_nutrition_api_200_response = require "apileague.model.compute_nutrition_api_200_response"
local apileague_retrieve_recipe_information_api_200_response = require "apileague.model.retrieve_recipe_information_api_200_response"
local apileague_search_drinks_api_200_response = require "apileague.model.search_drinks_api_200_response"
local apileague_search_recipes_api_200_response = require "apileague.model.search_recipes_api_200_response"
local apileague_search_restaurants_api_200_response = require "apileague.model.search_restaurants_api_200_response"
...
function food_api:compute_nutrition_api(ingredients, servings, reduce_oils)
	...
	if http_status:sub(1,1) == "2" then
		...
		return apileague_compute_nutrition_api_200_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
...
function food_api:retrieve_recipe_information_api(id, add_wine_pairing)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
	...
	if http_status:sub(1,1) == "2" then
		...
		return apileague_retrieve_recipe_information_api_200_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
...
function food_api:search_drinks_api(query, glass_types, flavors, diet, include_ingredients, exclude_ingredients, min_calories, max_calories, min_carbs, max_carbs, min_protein, max_protein, min_fat, max_fat, min_alcohol_percent, max_alcohol_percent, min_caffeine, max_caffeine, sort, sort_direction, offset, Number_)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/search-drinks?query=%s&glass-types=%s&flavors=%s&diet=%s&include-ingredients=%s&exclude-ingredients=%s&min-calories=%s&max-calories=%s&min-carbs=%s&max-carbs=%s&min-protein=%s&max-protein=%s&min-fat=%s&max-fat=%s&min-alcohol-percent=%s&max-alcohol-percent=%s&min-caffeine=%s&max-caffeine=%s&sort=%s&sort-direction=%s&offset=%s&number=%s",
			self.basePath, http_util.encodeURIComponent(query), http_util.encodeURIComponent(glass_types), http_util.encodeURIComponent(flavors), http_util.encodeURIComponent(diet), http_util.encodeURIComponent(include_ingredients), http_util.encodeURIComponent(exclude_ingredients), http_util.encodeURIComponent(min_calories), http_util.encodeURIComponent(max_calories), http_util.encodeURIComponent(min_carbs), http_util.encodeURIComponent(max_carbs), http_util.encodeURIComponent(min_protein), http_util.encodeURIComponent(max_protein), http_util.encodeURIComponent(min_fat), http_util.encodeURIComponent(max_fat), http_util.encodeURIComponent(min_alcohol_percent), http_util.encodeURIComponent(max_alcohol_percent), http_util.encodeURIComponent(min_caffeine), http_util.encodeURIComponent(max_caffeine), http_util.encodeURIComponent(sort), http_util.encodeURIComponent(sort_direction), http_util.encodeURIComponent(offset), http_util.encodeURIComponent(Number_));
	})
	...
	if http_status:sub(1,1) == "2" then
		...
		return apileague_search_drinks_api_200_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
...
function food_api:search_recipes_api(query, cuisines, exclude_cuisines, meal_type, diet, intolerances, equipment, include_ingredients, exclude_ingredients, fill_ingredients, add_recipe_information, max_time, min_servings, max_servings, min_calories, max_calories, min_carbs, max_carbs, min_protein, max_protein, min_fat, max_fat, min_sugar, max_sugar, min_fiber, max_fiber, min_folate, max_folate, min_folic_acid, max_folic_acid, min_iodine, max_iodine, min_iron, max_iron, min_zinc, max_zinc, min_magnesium, max_magnesium, min_manganese, max_manganese, min_phosphorus, max_phosphorus, min_potassium, max_potassium, min_sodium, max_sodium, min_selenium, max_selenium, min_copper, max_copper, min_calcium, max_calcium, min_choline, max_choline, min_cholesterol, max_cholesterol, min_fluoride, max_fluoride, min_alcohol, max_alcohol, min_caffeine, max_caffeine, min_saturated_fat, max_saturated_fat, min_vitamin_a, max_vitamin_a, min_vitamin_c, max_vitamin_c, min_vitamin_d, max_vitamin_d, min_vitamin_e, max_vitamin_e, min_vitamin_k, max_vitamin_k, min_vitamin_b1, max_vitamin_b1, min_vitamin_b2, max_vitamin_b2, min_vitamin_b3, max_vitamin_b3, min_vitamin_b5, max_vitamin_b5, min_vitamin_b6, max_vitamin_b6, min_vitamin_b12, max_vitamin_b12, sort, sort_direction, offset, Number_)
	...
	if http_status:sub(1,1) == "2" then
		...
		return apileague_search_recipes_api_200_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
...
function food_api:search_restaurants_api(lat, lon, query, distance, budget, min_rating, cuisine, is_open, page, sort)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
	...
	if http_status:sub(1,1) == "2" then
		...
		return apileague_search_restaurants_api_200_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
